...
> **Note:** This project currently runs fully **locally / via Docker Compose**. AWS deployment (EC2 + RDS + S3) is scoped and documented but not yet live — see [AWS Deployment (Planned)](#aws-deployment-planned).
...
**Why ROC-AUC and F1 as primary metrics (not accuracy):** the dataset is class-imbalanced (~26% churn). A model that predicts "No churn" for every customer scores ~74% accuracy while catching zero actual churners — useless for the business. ROC-AUC measures ranking quality across all thresholds; F1 balances precision and recall, which matters because both false negatives (missed churners) and false positives (wasted retention spend) carry real cost.
...
*(Insert your actual results table here after running training — do not use placeholder numbers in the final version.)*

| Model | Accuracy | Precision | Recall | F1 | ROC-AUC |
|---|---|---|---|---|---|
| _(fill in from your MLflow run)_ | | | | | |
...
Interactive Swagger docs available at `/docs` when the API is running.
